...
class UPSByPass(PythonDataSourcePlugin):
    ...
    def collect(self, config):
        ...
        d = yield getTableStuff(self._snmp_proxy, [upsBypassVoltage,
                                                   upsBypassCurrent,
                                                   upsBypassPower,
                                                    ])
    ...
    def onSuccess(self, result, config):
        ...
        for ds in config.datasources:
            ...
            voltage = result[upsBypassVoltage][upsBypassVoltage + '.' + snmpindex]
            data['values'][ds.component]['voltage'] = float(voltage)
            current = result[upsBypassCurrent][upsBypassCurrent + '.' + snmpindex]
            data['values'][ds.component]['current'] = float(current)
            power = result[upsBypassPower][upsBypassPower + '.' + snmpindex]
