...
class UPSInput(PythonDataSourcePlugin):
    ...
    def collect(self, config):
        ...
        d = yield getTableStuff(self._snmp_proxy, [upsInputFrequency,
                                                    upsInputVoltage,
                                                    upsInputCurrent,
                                                    upsInputTruePower,
                                                    ])
    ...
    def onSuccess(self, result, config):
        ...
        for ds in config.datasources:
            ...
            frequency = result[upsInputFrequency][upsInputFrequency + '.' + snmpindex]
            data['values'][ds.component]['frequency'] = float(frequency) / 10.0
            voltage = result[upsInputVoltage][upsInputVoltage + '.' + snmpindex]
            data['values'][ds.component]['voltage'] = float(voltage)
            current = result[upsInputCurrent][upsInputCurrent + '.' + snmpindex]
            data['values'][ds.component]['current'] = float(current)
            truepower = result[upsInputTruePower][upsInputTruePower + '.' + snmpindex]
            data['values'][ds.component]['truepower'] = float(truepower)
