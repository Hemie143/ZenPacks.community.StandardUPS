...
class UPSDevice(PythonDataSourcePlugin):
    ...
    def params(cls, datasource, context):
        ...
        """
        log.info('Starting StandardUPSDevice params')
        params = {}
    ...
    def collect(self, config):
        log.debug('Starting StandardUPSDevice collect')
        ...
        d = yield getScalarStuff(self._snmp_proxy, [upsBatteryStatus,
        ...
        log.debug('UPSDevice data:{}'.format(d))
        returnValue(d)
    ...
    def onSuccess(self, result, config):
        ...
        # TODO: Translate
        battery_status_props = self.battery_status_maps[result[upsBatteryStatus]]
        data['values'][None]['upsBatteryStatus'] = battery_status_props[1]
        data['events'].append({
            'device': config.id,
            'component': None,
        ...
        data['values'][None]['upsEstimatedChargeRemaining'] = result[upsEstimatedChargeRemaining]
        data['values'][None]['upsEstimatedMinutesRemaining'] = result[upsEstimatedMinutesRemaining]
        data['values'][None]['upsBatteryVoltage'] = result[upsBatteryVoltage] / 10
        data['values'][None]['upsBatteryCurrent'] = result[upsBatteryCurrent]
        data['values'][None]['upsInputLineBads'] = result[upsInputLineBads]
